...
/**
 * [ADMIN] Seed les témoignages par défaut si la BD est vide
 * POST /api/temoignages/admin/seed
 */
exports.seedTemoignages = async (req, res) => {
  try {
    const count = await Temoignage.countDocuments({});
    if (count > 0) {
      return res.json({ message: `ℹ️ La base contient déjà ${count} témoignage(s). Seed ignoré.`, seeded: 0 });
    }

    const defaults = [
      {
        auteur: { nom: 'Marie & Thomas', entreprise: '' },
        type: 'temoignage_externe',
        evenement: { type: 'Mariage' },
        titre: 'Une soirée absolument parfaite !',
        contenu: 'Randy a su créer une ambiance incroyable pour notre mariage. Les invités ont adoré, la musique était parfaitement choisie et la sono impeccable. Merci infiniment pour ce moment inoubliable.',
        note: 5, statut: 'approuve', isVisible: true, isFeatured: true
      },
      {
        auteur: { nom: 'Association Lumière', entreprise: 'Association Lumière' },
        type: 'temoignage_externe',
        evenement: { type: 'Conférence / Gala' },
        titre: 'Professionnalisme exemplaire',
        contenu: "Nous avons fait appel à ELIJAH'GOD pour notre gala annuel. Professionnalisme exemplaire, ponctualité, équipement de qualité. Tout s'est déroulé parfaitement. Nous renouvèlerons l'expérience sans hésiter.",
        note: 5, statut: 'approuve', isVisible: true, isFeatured: false
      },
      {
        auteur: { nom: 'Famille Dubois', entreprise: '' },
        type: 'temoignage_externe',
        evenement: { type: 'Anniversaire 50 ans' },
        titre: 'Dépassé toutes nos attentes !',
        contenu: "Pour les 50 ans de mon mari, j'avais besoin de quelqu'un de fiable et talentueux. Randy a dépassé toutes nos attentes ! L'ambiance était festive, conviviale et les surprises musicales ont ému toute la famille.",
        note: 5, statut: 'approuve', isVisible: true, isFeatured: false
      },
      {
        auteur: { nom: 'Église Nouvelle Vie', entreprise: 'Église Nouvelle Vie' },
        type: 'temoignage_externe',
        evenement: { type: 'Culte & Louange' },
        titre: 'Un véritable serviteur',
        contenu: 'Randy est un véritable serviteur. Sa sensibilité spirituelle et son expertise technique font de lui le partenaire idéal pour nos événements déglise. La sonorisation était claire et puissante, le tout dans un esprit de paix.',
        note: 5, statut: 'approuve', isVisible: true, isFeatured: true
      },
      {
        auteur: { nom: 'Sophie & Julien', entreprise: '' },
        type: 'temoignage_externe',
        evenement: { type: 'Mariage' },
        titre: 'La soirée dont nous rêvions',
        contenu: "Nous avons eu la chance de travailler avec Randy pour notre mariage champêtre. Son écoute, sa réactivité et sa créativité nous ont permis d'avoir exactement la soirée dont nous rêvions. Un grand merci du fond du cœur !",
        note: 5, statut: 'approuve', isVisible: true, isFeatured: false
      },
      {
        auteur: { nom: 'Entreprise InnoTech', entreprise: 'InnoTech SAS' },
        type: 'temoignage_externe',
        evenement: { type: "Séminaire d'entreprise" },
        titre: 'Service au top !',
        contenu: "Installation rapide, son parfait pour nos deux jours de conférence. L'équipe d'ELIJAH'GOD a géré tout le côté technique avec brio nous permettant de nous concentrer sur notre contenu.",
        note: 5, statut: 'approuve', isVisible: true, isFeatured: false
      },
    ];

    const created = await Temoignage.insertMany(defaults);
    res.json({ message: `✅ ${created.length} témoignages initialisés avec succès.`, seeded: created.length });
  } catch (error) {
